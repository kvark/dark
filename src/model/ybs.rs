...
pub mod test {
	use std::{io, rand, vec};
	use compress::entropy::ari;
	use super::super::{Distance, DistanceModel};
	use super::super::Symbol;

	fn roundtrip(input: &[(Symbol,Distance)]) {
		let mut m: super::Model = DistanceModel::new_default();
	...
	#[test]
	fn roundtrips() {
		roundtrip(gen_data(5,3));
		roundtrip(gen_data(1000,200));
	}
}
